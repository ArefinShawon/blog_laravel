            ...
            @foreach($categoryPost as $catPost)
                ...
                <div class="col-lg-3 col-md-6 mb-4 shadow-sm">
                    <div class="card h-100">
                        <img class="card-img-top" src="{{asset($catPost->post_img)}}" height="200px" alt="">
                        <div class="card-body shadow-sm">
                            <h4 class="card-title">{{$catPost->post_title}}</h4>
                            <p class="card-text small">{{str_limit($catPost->post_desc)}}</p>
                        </div>
                        <div class="card-footer shadow-sm">
                            <a href="#" class="">Continue reading</a>
                            <hr>
                            <p class="small">
                                By {{$catPost->post_author}} at {{\Carbon\Carbon::parse($catPost->created_at)->format('D,d M Y')}}
                            </p>
                        </div>
                    </div>
                </div>
                @else
                    <p>Sorry, {{$catPost->cat_name}} category has no post</p>
                @endif
            @endforeach
        </div>
    </div>
